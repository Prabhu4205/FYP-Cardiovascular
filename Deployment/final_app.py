...
if uploaded_file is not None:
    ...
    my_expander1 = st.expander(label='DIVIDING LEADS')
    with my_expander1:
    ...
    my_expander2 = st.expander(label='PREPROCESSED LEADS')
    with my_expander2:
        st.image('Preprocessed_Leads_1-12_figure.png')
        st.image('Preprocessed_Lead_13_figure.png')

    """#### **EXTRACTING SIGNALS (1-12 & 13)**"""
    # call the signal extraction method
    ec_signal_extraction = ecg.SignalExtraction_Scaling(dividing_leads)

    # streamlit expander for contour leads
    my_expander3 = st.expander(label='CONTOUR LEADS')
    with my_expander3:
        st.image('Contour_Leads_1-12_figure.png')
        st.image('Contour_Lead_13_figure.png')
